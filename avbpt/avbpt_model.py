...
class AVBPTree(model.Tree):
    ...
    def clear(self):
        """ Additionally resets AVBPTNode.class_node_id """
        self.root = None
        AVBPTNode.class_node_id = ord('@')


class AVBPTValue(model.AnimatedObject):
...
class AVBPTNode(model.AnimatedObject, model.Node):
    ...
    @staticmethod
    def get_id():
        AVBPTNode.class_node_id += 1
        if AVBPTNode.class_node_id == ord('['):
            AVBPTNode.class_node_id = ord('a')
        elif AVBPTNode.class_node_id == ord('{'):
            AVBPTNode.class_node_id = ord('A')
        return chr(AVBPTNode.class_node_id)
    ...
    def fix_insert(self):
        ...
        if self.parent is not None:
            ...
            s = AVBPTNode(self.tree, False, self.tree.view.width // 2, self.tree.view.y_space)
