...
class AggTree(model.Tree):
    ...
    def insert_value(self, value):  # POT
        if self.root is None:
            ...
            self.root.values.append(AggValue(value, self.root))
        else:
            view = self.view
            ...
            self.root.insert_value(AggValue(value, None, self.root.x, self.root.y - view.y_above))
...
class AggValue(model.AnimatedObject):
...
class AggNode(model.AnimatedObject, model.Node):
    ...
    def __init__(self, tree, is_leaf, x, y):
        ...
        self.values = []
        self.children = []
        self.id = self.tree.node_class.get_id()
