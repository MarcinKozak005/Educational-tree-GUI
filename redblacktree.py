import tkinter as tk
from tkinter import font
from tkinter.constants import X
import root as r
import menu as m

class rb_node():
    def __init__(self,x,y,val,l_edge,r_edge,parent):
        self.x = x
        self.y = y
        self.val = val
        self.l_edge = l_edge
        self.r_edge = r_edge
        self.color = "red"
        self.left = rb_leaf()
        self.right = rb_leaf()
        self.parent = parent
    # def __repr__(self):
    #     return f"({self.text} [{self.a},{self.b}])"

class rb_leaf():
    color = 'black'
    def __init__(self):
        pass


rb_tree = None
y_space = 30
width = 800
node_size = 26
half_node_size = node_size/2

def clear():
    global rb_tree
    rb_tree = None # on se tu radzi z pamięcia, co nie?
    canvas.delete("all")
    print('########')

# można zrobić dummy func zeby nie przekazywac canvasa cały czas
# tam tez check 0-999
def rb_tree_add(val, tree, canvas): 
    if val >= tree.val and type(tree.right) != rb_leaf:
        print(f'add-1 {tree.val}')
        newNode = rb_tree_add(val,tree.right,canvas)
    elif val >= tree.val:
        unit = (tree.r_edge - tree.l_edge)/4
        newNode = rb_node(tree.x + unit,tree.y + y_space,val,tree.x,tree.r_edge,tree)
        tree.right = newNode
        print(f'add-2 {tree.val}')
    elif val < tree.val and type(tree.left) != rb_leaf:
        print(f'add-3 {tree.val}')
        newNode = rb_tree_add(val,tree.left, canvas)
    else:
        unit = (tree.r_edge - tree.l_edge)/4
        newNode = rb_node(tree.x - unit,tree.y + y_space,val,tree.l_edge,tree.x,tree)
        tree.left = newNode
        print(f'add-4 {tree.val}')
    return newNode

def rb_tree_add_hat(text,tree,canvas):
    global rb_tree
    if rb_tree is None:
        rb_tree = rb_node(width//2,y_space,int(text),0,width,None)
        rb_tree.color = 'black'
        rb_tree.left = rb_leaf()
        rb_tree.right = rb_leaf()
    else:
    # polepsz to te ciagłe zmiany do int
        val = int(text) 
        newNode = rb_tree_add(val,tree,canvas)
        fix_rb_tree(newNode)
    canvas.delete("all")
    draw_rb_tree(rb_tree,canvas)
    print_tree(rb_tree)
    print('-------')


def fix_rb_tree(node):
    while node is not rb_tree and node.parent.color == 'red':
        if node.parent == node.parent.parent.left:
            y = node.parent.parent.right
            if y.color == 'red':
                node.parent.color = 'black'
                y.color = 'black'
                node.parent.parent.color = 'red'
                node = node.parent.parent
                print(f'fix-1 {node.val}')
            elif node == node.parent.right:
                node = node.parent
                left_rotate(node)
                print(f'fix-2 {node.val}')
            if node is not rb_tree and node.parent is not rb_tree:
                print(f'{node.val}-{node.parent.val}-{node.parent.parent.val}')
                node.parent.color = 'black'
                node.parent.parent.color = 'red'
                right_rotate(node.parent.parent)
                print(f'fix-3 {node.val}')
        else:
            y = node.parent.parent.left
            if y.color == 'red':
                node.parent.color = 'black'
                y.color = 'black'
                node.parent.parent.color = 'red'
                node = node.parent.parent
                print(f'fix-4 {node.val}')
            elif node == node.parent.left:
                node = node.parent
                right_rotate(node)
                print(f'fix-5 {node.val}')
            if node is not rb_tree and node.parent is not rb_tree:
                print(f'{node.val}-{node.parent.val}-{node.parent.parent.val}')
                node.parent.color = 'black'
                node.parent.parent.color = 'red'
                left_rotate(node.parent.parent)
                print(f'fix-6 {node.val}')
    rb_tree.color = 'black'

def left_rotate(node):
    global rb_tree
    y = node.right
    node.right = y.left
    if type(y.left) != rb_leaf:
        y.left.parent = node
        print(f'lr-1 {node.val}')
    y.parent = node.parent
    if node.parent == None:
        rb_tree = y
        print(f'lr-2 {node.val}')
    elif node == node.parent.left:
        node.parent.left = y
        print(f'lr-3 {node.val}')
    else:
        node.parent.right = y
        print(f'lr-4 {node.val}')
    y.left = node
    node.parent = y
    update_positions(y)


def right_rotate(node):
    global rb_tree
    y = node.left
    node.left = y.right
    if type(y.right) != rb_leaf:
        y.right.parent = node
        print(f'rr-1 {node.val}')
    y.parent = node.parent
    if node.parent == None:
        rb_tree = y
        print(f'rr-2 {node.val}')
    elif node == node.parent.right:
        node.parent.right = y
        print(f'rr-3 {node.val}')
    else:
        node.parent.left = y
        print(f'rr-4 {node.val}')
    y.right = node
    node.parent = y
    update_positions(y)

# newNode = rb_node(tree.x + unit,tree.y + y_space,val,tree.x,tree.r_edge,tree)
def update_positions(node):
    if type(node) != rb_leaf and node is not rb_tree:
        unit = (node.parent.r_edge - node.parent.l_edge)/4
        if node is node.parent.right:
            node.x = node.parent.x + unit
            node.y = node.parent.y + y_space
            node.l_edge = node.parent.x
            node.r_edge = node.parent.r_edge
        elif node is node.parent.left:
            node.x = node.parent.x - unit
            node.y = node.parent.y + y_space
            node.l_edge = node.parent.l_edge
            node.r_edge = node.parent.x
    elif node is rb_tree:
        node.x = width//2
        node.y = y_space
        node.l_edge = 0
        node.r_edge = width
    if type(node) != rb_leaf:
        update_positions(node.left)
        update_positions(node.right)


def draw_rb_tree(tree, canvas):
    draw_rb_node(tree,canvas)
    if type(tree) != rb_leaf:
        draw_rb_tree(tree.left,canvas)
        draw_rb_tree(tree.right, canvas)

def draw_rb_node(node, canvas):
    if type(node) != rb_leaf:
        canvas.create_oval(node.x-half_node_size, node.y-half_node_size, node.x+half_node_size, node.y+half_node_size, fill=node.color)
        canvas.create_text(node.x,node.y,fill="white",text=node.val)

frame = tk.Frame(r.frame)
tk.Label(frame,text="RedBlack Tree",bg='red', height=2).pack(fill='x')
i = tk.Label(frame,pady=5)
e = tk.Entry(frame)
canvas = tk.Canvas(frame,width=width, height=width//2, bg="white")
b = tk.Button(frame,text="Draw text", command=lambda: rb_tree_add_hat(e.get(),rb_tree,canvas),pady=5)
b2 = tk.Button(frame,text="Clear", command=lambda:clear())

e.pack()
b.pack()
b2.pack()
i.pack()
canvas.pack()
tk.Button(frame,text='Back',command=lambda: r.show_frame(m.frame)).pack()


def print_tree(tree,i=0):
    if type(tree) is not rb_leaf:
        print(' '*i + f'{tree.val}')
        i += 1
        print_tree(tree.left,i)
        print_tree(tree.right,i)
        i -= 1
    